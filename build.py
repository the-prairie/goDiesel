...
@media (max-width: 700px) {
  ...
  .route-lock { order: 6; flex: 1 1 calc(50% - 4px); }
...
let cinemaPoints = [], cinemaSlug = null;
let allPhotos = [];
...
function syncRouteCinemaButton() {
  ...
  if (layer) layer.classList.toggle('active', routeCinemaEnabled);
}
...
function initRouteCinema(route) {
  ...
  cinemaScene.fog = new THREE.Fog(0x030709, 8, 18);
  ...
  const ambient = new THREE.AmbientLight(0x9fd8c9, 1.2);
  const key = new THREE.DirectionalLight(0xffffff, 1.7);
  key.position.set(3, 7, 5);
  cinemaScene.add(ambient, key);

  const floor = new THREE.Mesh(
    new THREE.PlaneGeometry(13, 13, 18, 18),
    new THREE.MeshStandardMaterial({ color: 0x071115, roughness: 0.82, metalness: 0.05, transparent: true, opacity: 0.76, wireframe: true })
  );
  ...
  cinemaFullLine = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints(cinemaPoints),
    new THREE.LineBasicMaterial({ color: 0xbfeee2, transparent: true, opacity: 0.28 })
  );
  cinemaScene.add(cinemaFullLine);
  cinemaProgressLine = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints([cinemaPoints[0] || new THREE.Vector3()]),
  ...
  cinemaMarker = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0x00f19f, emissiveIntensity: 0.9 })
  );
...
function updateRouteCinema(route, idx) {
  ...
  cinemaProgressLine.geometry = new THREE.BufferGeometry().setFromPoints(cinemaPoints.slice(0, safeIdx + 1));
  ...
  cinemaScene.rotation.y = -0.18 + (safeIdx / Math.max(cinemaPoints.length - 1, 1)) * 0.36;
  cinemaRenderer.render(cinemaScene, cinemaCamera);
}
