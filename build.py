...
.cinema-layer.active[data-mode="flyover"] {
  background: radial-gradient(circle at 45% 42%, rgba(0,147,231,0.16), transparent 36%),
              linear-gradient(180deg, rgba(4,10,14,0.98), rgba(3,7,9,0.99)); }
...
.route-cinema.active { color: #FFF; border-color: rgba(0,147,231,0.85); background: rgba(0,147,231,0.12); }
  ...
  <div class="stage">
    <div class="panel-map">
      ...
      <div class="scrubber-wrap">
        ...
        <button class="route-control route-cinema" id="routeCinemaBtn" type="button" onclick="toggleRouteCinema()" title="Toggle immersive 3D route preview">CINEMA</button>
...
const requestedCinemaMode = new URLSearchParams(location.search).get('lab');
...
function syncRouteCinemaButton() {
  ...
  if (modeBtn) {
    ...
    modeBtn.textContent = routeCinemaMode === 'quest' ? 'QUEST' : routeCinemaMode.toUpperCase();
  }
  if (layer) {
...
function initRouteCinema(route) {
  ...
  const ambient = new THREE.AmbientLight(routeCinemaMode === 'quest' ? 0xb8f0d8 : 0x9fd8c9, 1.18);
  const key = new THREE.DirectionalLight(0xffffff, routeCinemaMode === 'flyover' ? 1.95 : 1.55);
  key.position.set(3, 7, 5);
  const rim = new THREE.DirectionalLight(0x00f19f, routeCinemaMode === 'artifact' ? 0.55 : 0.9);
  ...
  const floor = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
      ...
      metalness: 0.05,
      transparent: true,
      opacity: routeCinemaMode === 'flyover' ? 0.42 : 0.76,
  ...
  if (routeCinemaMode !== 'artifact') {
    const grid = new THREE.GridHelper(13, routeCinemaMode === 'quest' ? 18 : 13, 0x00f19f, 0x2d4d4b);
  ...
  cinemaFullLine = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0x00f19f, transparent: true, opacity: routeCinemaMode === 'artifact' ? 0.96 : 0.86 })
  );
  ...
  cinemaProgressLine = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: routeCinemaMode === 'quest' ? 0xffd36a : 0x00f19f, transparent: true, opacity: 0.98 })
  );
  cinemaScene.add(cinemaProgressLine);
  cinemaMarker = new THREE.Mesh(
    new THREE.SphereGeometry(routeCinemaMode === 'quest' ? 0.22 : 0.18, 24, 16),
    new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: routeCinemaMode === 'quest' ? 0xffb000 : 0x00f19f, emissiveIntensity: 0.9 })
  );
  cinemaScene.add(cinemaMarker);
  const markerHalo = new THREE.Mesh(
    new THREE.SphereGeometry(routeCinemaMode === 'quest' ? 0.42 : 0.32, 24, 16),
    new THREE.MeshBasicMaterial({ color: routeCinemaMode === 'quest' ? 0xffd36a : 0x00f19f, transparent: true, opacity: 0.14 })
  );
...
function buildCinemaDecor(route) {
  ...
  if (routeCinemaMode === 'flyover') {
    ...
    add(new THREE.Line(
      ...
      new THREE.LineBasicMaterial({ color: 0x00f19f, transparent: true, opacity: 0.26 })
    ));
    const horizon = new THREE.Mesh(
      new THREE.RingGeometry(4.8, 5.0, 80),
      new THREE.MeshBasicMaterial({ color: 0x0093e7, transparent: true, opacity: 0.12, side: THREE.DoubleSide })
    );
