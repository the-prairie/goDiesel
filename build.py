...
.cinema-layer.active[data-mode="flyover"] {
  background: radial-gradient(circle at 46% 42%, rgba(225,203,150,0.18), transparent 36%),
              radial-gradient(circle at 60% 62%, rgba(0,241,159,0.11), transparent 42%),
              linear-gradient(180deg, rgba(12,17,16,0.82), rgba(6,10,10,0.90)); }
...
.cinema-memory { position: absolute; opacity: 0; width: 126px;
                 transform: translate(-50%, -112%) scale(0.92) rotate(-1deg);
                 transition: opacity 240ms ease, transform 240ms ease;
                 background: rgba(245,235,212,0.10); border: 1px solid rgba(232,212,154,0.30);
                 border-radius: 10px; padding: 6px;
                 box-shadow: 0 18px 42px rgba(0,0,0,0.34), 0 0 24px rgba(232,212,154,0.08); }
...
const ROUTE_CINEMA_MODES = ['artifact', 'flyover', 'quest'];
...
const requestedCinemaModeRaw = new URLSearchParams(location.search).get('lab');
...
function syncRouteCinemaButton() {
  ...
  if (btn) btn.classList.toggle('active', routeCinemaEnabled);
  if (modeBtn) {
    modeBtn.classList.toggle('visible', routeCinemaEnabled);
  ...
  if (label) {
    label.textContent = {
      ...
      flyover: 'Memory Atlas · real route, remembered places',
...
function initRouteCinema(route) {
  ...
  const ambient = new THREE.AmbientLight(routeCinemaMode === 'flyover' ? 0xf1dfb4 : (routeCinemaMode === 'quest' ? 0xb8f0d8 : 0x9fd8c9), 1.18);
  const key = new THREE.DirectionalLight(routeCinemaMode === 'flyover' ? 0xfff6dc : 0xffffff, routeCinemaMode === 'flyover' ? 1.55 : 1.55);
  key.position.set(3, 7, 5);
  const rim = new THREE.DirectionalLight(routeCinemaMode === 'flyover' ? 0xe8d49a : 0x00f19f, routeCinemaMode === 'artifact' ? 0.55 : 0.9);
  ...
  const floor = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
      ...
      metalness: 0.05,
      transparent: true,
      opacity: routeCinemaMode === 'flyover' ? 0.24 : 0.76,
      wireframe: routeCinemaMode !== 'flyover'
    })
  );
  ...
  if (routeCinemaMode !== 'artifact') {
    ...
    grid.material.opacity = routeCinemaMode === 'flyover' ? 0.055 : (routeCinemaMode === 'quest' ? 0.18 : 0.1);
    cinemaScene.add(grid);
  }

  cinemaFullLine = new THREE.Mesh(
    cinemaRouteGeometry(cinemaPoints, routeCinemaMode === 'quest' ? 0.046 : 0.038),
    new THREE.MeshBasicMaterial({ color: routeCinemaMode === 'flyover' ? 0xbaf5dc : 0x00f19f, transparent: true, opacity: routeCinemaMode === 'artifact' ? 0.96 : 0.78 })
  );
  cinemaScene.add(cinemaFullLine);
  cinemaProgressLine = new THREE.Mesh(
    cinemaRouteGeometry(cinemaPoints.slice(0, 2), routeCinemaMode === 'quest' ? 0.065 : 0.055),
    new THREE.MeshBasicMaterial({ color: routeCinemaMode === 'flyover' ? 0xe8d49a : (routeCinemaMode === 'quest' ? 0xffd36a : 0x00f19f), transparent: true, opacity: 0.98 })
  );
  cinemaScene.add(cinemaProgressLine);
  cinemaMarker = new THREE.Mesh(
    new THREE.SphereGeometry(routeCinemaMode === 'flyover' ? 0.15 : (routeCinemaMode === 'quest' ? 0.22 : 0.18), 24, 16),
    new THREE.MeshStandardMaterial({ color: 0xfff7df, emissive: routeCinemaMode === 'flyover' ? 0xe8d49a : (routeCinemaMode === 'quest' ? 0xffb000 : 0x00f19f), emissiveIntensity: routeCinemaMode === 'flyover' ? 0.42 : 0.9 })
  );
  cinemaScene.add(cinemaMarker);
  const markerHalo = new THREE.Mesh(
    new THREE.SphereGeometry(routeCinemaMode === 'flyover' ? 0.28 : (routeCinemaMode === 'quest' ? 0.42 : 0.32), 24, 16),
    new THREE.MeshBasicMaterial({ color: routeCinemaMode === 'flyover' ? 0xe8d49a : (routeCinemaMode === 'quest' ? 0xffd36a : 0x00f19f), transparent: true, opacity: 0.14 })
  );
...
function buildCinemaDecor(route) {
  ...
  if (routeCinemaMode === 'flyover') {
    const shadowPoints = cinemaPoints.map(p => new THREE.Vector3(p.x, -0.69, p.z));
    add(new THREE.Line(
      new THREE.BufferGeometry().setFromPoints(shadowPoints),
      new THREE.LineBasicMaterial({ color: 0xbaf5dc, transparent: true, opacity: 0.16 })
    ));
    const horizon = new THREE.Mesh(
      new THREE.RingGeometry(4.8, 5.0, 80),
      new THREE.MeshBasicMaterial({ color: 0xe8d49a, transparent: true, opacity: 0.07, side: THREE.DoubleSide })
    );
    ...
    routeMomentSpecs(route).forEach((moment, i) => {
      ...
      const ring = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: moment.key === 'high' ? 0xe8d49a : 0xbaf5dc, transparent: true, opacity: 0.16 })
      );
    ...
    for (let i = 0; i < 52; i++) {
      ...
      const mote = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: i % 6 === 0 ? 0xe8d49a : 0xbaf5dc, transparent: true, opacity: 0.10 })
      );
      mote.position.set(
...
function updateCinemaMomentLabels(safeIdx) {
  ...
  const placeElement = (item, point, offset = 0, minTop = 84) => {
    if (!point || !item.el) return false;
    const projected = point.clone().project(cinemaCamera);
    const visible = Math.abs(safeIdx - item.idx) <= proximity && projected.z < 1;
    item.el.classList.toggle('active', visible);
    if (visible) {
      const x = (projected.x * 0.5 + 0.5) * rect.width;
      const y = (-projected.y * 0.5 + 0.5) * rect.height;
      item.el.style.left = `${Math.max(86, Math.min(rect.width - 86, x + offset))}px`;
      item.el.style.top = `${Math.max(minTop, Math.min(rect.height - 132, y))}px`;
    }
    return visible;
  };
  cinemaMoments.forEach(moment => {
    placeElement(moment, cinemaPoints[moment.idx]);
  });
  ...
  cinemaMemories.forEach((memory, i) => {
    ...
    placeElement(memory, cinemaPoints[memory.idx], i % 2 ? 46 : -46, 176);
  });
}
