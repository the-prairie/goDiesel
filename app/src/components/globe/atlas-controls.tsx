...
export function AtlasControls({
  ...
  return (
    <>
      <div className="absolute left-4 top-4 z-20 flex items-center gap-2 rounded-sm border border-white/35 bg-[#f6f2e8]/92 p-2 text-[#24322d] shadow-lg backdrop-blur md:left-5 md:top-5">
        ...
        <select
          ...
          className="min-h-10 min-w-0 max-w-52 rounded-sm border border-[#c7c1b5] bg-[#fffdf8] px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-[#315fb4]"
        >
      ...
      <div className="absolute right-4 top-4 z-20 hidden rounded-sm border border-white/35 bg-[#f6f2e8]/92 p-1 shadow-lg backdrop-blur sm:flex xl:hidden" aria-label="Activity filter">
        {(["all", "runs", "rides"] as const).map((value) => (
          <Button
            key={value}
            type="button"
      ...
      <div className="absolute bottom-4 left-4 z-20 flex gap-1 rounded-sm border border-white/35 bg-[#f6f2e8]/92 p-1 shadow-lg backdrop-blur md:bottom-5 md:left-5">
