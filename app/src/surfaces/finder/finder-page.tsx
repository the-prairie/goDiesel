...
export function FinderPage() {
  ...
  return (
    <section className="relative h-[calc(100dvh-var(--mobile-navigation-height))] min-h-0 overflow-hidden bg-[#cadfdc] md:h-dvh">
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-20 p-3 md:right-auto md:w-[25rem] md:p-5">
        <div className="pointer-events-auto border border-white/65 bg-surface/90 p-3 shadow-panel backdrop-blur-xl md:p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 hidden text-control text-ink-secondary sm:block">
          ...
          {submittedIntent ? (
            <div className="mt-2 flex min-w-0 items-center justify-between gap-2 border-t border-line/80 pt-2 md:mt-3 md:gap-3 md:pt-3">
              ...
              <Button type="button" variant="ghost" size="sm" className="shrink-0 px-2 md:px-3" aria-label="Edit filters" onClick={() => setFiltersOpen(true)}>
                <SlidersHorizontal aria-hidden="true" /> <span className="hidden md:inline">Edit filters</span>
              </Button>
            </div>
          ) : (
            <Button type="button" className="mt-4 w-full" onClick={() => setFiltersOpen(true)}>
...
function ActiveFilterChips({ intent, onRemove }: { intent: FinderIntent; onRemove: (filter: "place" | "distance" | "terrain" | "vibe") => void }) {
  ...
  return (
    <div aria-label="Active Finder filters" className="flex min-w-0 flex-1 gap-1.5 overflow-x-auto">
      {chips.map((chip) => (
...
function FinderState({ title, copy, role }: { title: string; copy: string; role?: "status" }) {
  return (
