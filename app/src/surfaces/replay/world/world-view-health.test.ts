...
const full = { sampledAtMs: 1000, tested: 15, hits: 15, centerHit: true };
describe("current landscape coverage", () => {
  ...
  it("intersects actual selected geometry, excluding cached invisible models", () => {
    ...
    const camera = new PerspectiveCamera(50,1,0.1,100); camera.position.z = 10; camera.up.set(0,0,1); camera.updateMatrixWorld();
    ...
    camera.near = 11; camera.updateProjectionMatrix();
    expect(sampleWorldView(tiles,camera,1000).centerHit).toBe(false);
    camera.near = 0.1; camera.far = 9; camera.updateProjectionMatrix();
    expect(sampleWorldView(tiles,camera,1000).hits).toBe(0);
